...
func makeHeaderChain(parent *types.Header, n int, db fdgdb.Database, seed int) []*types.Header {
	blocks, _ := core.GenerateChain(params.TestChainConfig, types.NewBlockWithHeader(parent), fdgash.NewFaker(), db, n, func(i int, b *core.BlockGen) {
...
func newCanonical(n int) (fdgdb.Database, *LightChain, error) {
	...
	blockchain, _ := NewLightChain(&dummyOdr{db: db, indexerConfig: TestClientIndexerConfig}, gspec.Config, fdgash.NewFaker(), nil)
...
func newTestLightChain() *LightChain {
	...
	gspec.MustCommit(db)
	lc, err := NewLightChain(&dummyOdr{db: db}, gspec.Config, fdgash.NewFullFaker(), nil)
	if err != nil {
		panic(err)
	}
	return lc
...
func TestReorgBadHeaderHashes(t *testing.T) {
	...
	ncm, err := NewLightChain(&dummyOdr{db: bc.chainDb}, params.TestChainConfig, fdgash.NewFaker(), nil)
	if err != nil {
